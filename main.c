...
#include<conio.h>
#include<windows.h>
struct dat//for date(mm dd)
{
    int d,m;//day, month
};
int clscanf();//class 1-12
struct student
{
    struct dat dt;
...
void addrec(int);//add record
void modrec(int);//modify record
void searchrec(int);//search for records
void delrec(int);//delete record
void salary(int);//teacher/staff salary
...
void addrec(int j)
{
    int dif,cdat,ddat,month=0;//cdat=fee due date
    ...
    if (j==1)
    {
        while(c=='y'||c=='Y')
        {
            ...
            fflush(stdin);
            scanf("%[^\n]",stud.n);
            printf("\nEnter the class (1-12): ");
            fflush(stdin);
            stud.c=clscanf();
            printf("\nEnter the Roll No.:");
            fflush(stdin);
            scanf("%2d",&stud.r);
            printf("\nEnter fee due date (MMDD):");
            fflush(stdin);
            ...
            if (choice==1)
            {
                ...
                while(fread(&stud,sizeof(stud),1,fs)==1)
                {
                    ...
                    if (strcmpi(name,stud.n)==0)
                    {
                        ...
                        fflush(stdin);
                        scanf("%[^\n]",stud.n);
                        printf("\nenter students new class: ");
                        fflush(stdin);
                        stud.c=clscanf();
                        printf("\nenter students new roll No.: ");
                        fflush(stdin);
                        scanf("%d",&stud.r);
    ...
    if (j==2)
    {
        while(c=='y'||c=='n')
        {
            ...
            while(fread(&tech,sizeof(tech),1,ft)==1)
            {
                ...
                {
                    a=0;
                    printf("\nenter teachers new name: ");
                    fflush(stdin);
                    scanf("%[^\n]",tech.n);
                    printf("\nenter teachers new id: ");
                    fflush(stdin);
                    scanf("%d",&tech.id);
    ...
    while(c=='y'||c=='n')
    {
        ...
        while(fread(&tech,sizeof(tech),1,f)==1)
        {
            if(strcmp(tech.n,name)==0 )
            {
                float lsal;
                a=0;
                printf("\n\nEnter the date of last salary payment (DDMM): ");
                fflush(stdin);
                scanf("%d",&month);
...
void fee(int mm)
{
    ...
    while(c=='y'||c=='n')
    {
        ...
        while(fread(&stud,sizeof(stud),1,f)==1)
        {
            ...
            {
                a=0;
                printf("\n\nEnter the month the fee is due: ");
                fflush(stdin);
                scanf("%d",&month);
                month=chkdat(month,day);
                dif=mm-stud.dt.m;
